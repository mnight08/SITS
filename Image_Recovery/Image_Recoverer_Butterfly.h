class ImageRecovererButterfly : ImageRecoverer {
  ...
  complex<double> f(point y, complex<double> *d) {
    return (conj(p(w_0 - Band / 2 + y.y * Band)) * interp(d, y.x, y.y)) /
           ((w_0 - Band / 2 + y.y * Band) *
            abs(p(w_0 - Band / 2 + y.y * Band)) *
            abs(p(w_0 - Band / 2 + y.y * Band)));
  }
  // Normalized Phase function.  All arguments are in $[0,1]^2$
  double phi(point x, point y) {
    return -(w_0 - Band / 2 + y.y * Band) *
           (2 * (Gamma_X(y.x) * x.x + Gamma_Y(y.x) * x.y - r) / c_0);
    ...
    inline double L1d(Grid grid, int l, int Box, int t, double x,
                      ostream &log) {
      double product = 1;
    ...
    double phi(point x, point y) {
      ...
      // We always count from left to right, bottom up, so that we would start
      // around (0,0), and end around (1,1)
      ...
      inline int Parent(int A, ostream &log) { return A >> 2; }

      // Children of B are numbers where we append two bits
      inline int Child(int B, int c, ostream &log) { return (B << 2) + c; }
      // There are N*N*r_eps weights to index.  Here, B needs to have at most 2l
      // bits
      ...
      // The unit box is naturally partitioned. The box that a point is in is
      // completely determined by  the first 2l bits of its components. First l
      // (from left) bits are where y is, next l are x.
      ...
      inline int Image_Index(int m, int n) { return N * n + m; }
      inline int Data_Index(int s, int w) { return N * s + w; }
      inline double X(int m) { return m / (double)N + 1.0 / (2.0 * N); }
      inline double Y(int n) { return n / (double)N + 1.0 / (2.0 * N); }
      inline double Gamma_D_X(double sl) {
        return Gamma_X(sl) /
               sqrt(Gamma_X(sl) * Gamma_X(sl) + Gamma_Y(sl) * Gamma_Y(sl));
      }
      ...
      inline double Q(int e) { return e / N + 1.0 / (2 * N); }
      inline double S(int s) { return s / (double)N + 1.0 / (2 * N); }
      ...
      // Flight parameters are defined in a SAR object. Conventions are handled
      // by functions above.  We follow the algorithm developed in: "A butterfly
      // algorithm for synthetic aperture radar imaging"  At each level there
      // are N^2q^2 weights to compute. We need weights from the previous level
      // to compute weights at current level.
      void Image_Recovery_Butterfly(double *&recovered_reflectivity,
        ...
        // Step 1: Initialize the weights. Done different than the paper to not
        // need tree.  go through all the points that we are integrating over
        // and add them to the weight  for the box they are in. This is so we do
        // not need to have an actual tree structure  The exponential is moved
        // inside the sum.
        Zero(weights);
              ...
              for (int t1 = 0; t1 < q; t1++) {
                for (int t2 = 0; t2 < q; t2++) {
                  weights[Weight_Index(0, Box(y, L), l, t1, t2)] +=
                      exp(-i * (phi(Center(0, 0),
                                    Grid_Point(grids, Box(y, L), t1, t2, l)) -
                                phi(Center(0, 0), y))) *
                      L2d(grids, t1, t2, Box(y, L), L, y) * f(y, data);
                }
              }
            }
          }
        ...
        Zero(weights);
        // Step 2: for l=1,..., L/2
        // Compute the next weights from previous weights using the fact when
        // diam(B)<1/N^2
        for (l = 1; l <= L / 2; l++) {
          log << "Current level is" << l << endl;
          // go through all the pairs of boxes.  There are N^2.
          for (int n = 0; n < M; n++) {
            // go through the children of B paired with the parent of A to
            // compute weights
            for (int t1 = 0; t1 < q; t1++) {
              for (int t2 = 0; t2 < q; t2++) {
                for (int c = 0; c < 4; c++) {
                  ...
                  for (int tc1 = 0; tc1 < q; tc1++) {
                    for (int tc2 = 0; tc2 < q; tc2++) {
                      if (abs(previous_weights[Weight_Index(
                              Parent(A(n, l)), Child(B(n, l), c), l - 1, tc1,
                              tc2)]) != 0) {
                        weights[Weight_Index(A(n, l), B(n, l), l, t1, t2)] +=
                            L2d(grids, tc1, tc2, B(n, l), L - l,
                                Grid_Point(grids, Child(B(n, l), c), tc1, tc2,
                                           L - sl + 1)) *
                            exp(i * phi(Center(A(n, l), l),
                                        Grid_Point(grids, Child(B(n, l), c),
                                                   tc1, tc2, L - l + 1))) *
                            previous_weights[Weight_Index(Parent(A(n, l)),
                                                          Child(B(n, l), c),
                                                          l - 1, tc1, tc2)];
                      }
                    }
                ...
                weights[Weight_Index(A(n, l), B(n, l), l, t1, t2)] *=
                    exp(-i * phi(Center(A(n, l), l),
                                 Grid_Point(grids, B(n, l), t1, t2, L - l)));
              }
            }
          }
          Swap(previous_weights, weights);
            ...
            for (int t2 = 0; t2 < q; t2++) {
              for (int s1 = 0; s1 < q; s1++) {
                for (int s2 = 0; s2 < q; s2++) {
                  if (abs(previous_weights[Weight_Index(
                          A(n, L / 2), B(n, L / 2), L / 2, s1, s2)]) != 0) {
                    weights[Weight_Index(A(n, L / 2), B(n, L / 2), L / 2, t1,
                                         t2)] +=
                        exp(i * phi(Grid_Point(grids, A(n, l), t1, t2, l),
                                    Grid_Point(grids, B(n, l), s1, s2, l))) *
                        previous_weights[Weight_Index(A(n, L / 2), B(n, L / 2),
                                                      L / 2, t1, t2)];
                  }
                }
              }
                ...
                for (int c = 0; c < 4; c++) {
                  ...
                  for (int tp1 = 0; tp1 < q; tp1++) {
                    for (int tp2 = 0; tp2 < q; tp2++) {
                      ...
                      if (abs(previous_weights[Weight_Index(
                              Parent(A(n, l)), Child(B(n, l), c), l - 1, tp1,
                              tp2)]) != 0) {
                        weights[Weight_Index(A(n, l), B(n, l), l, t1, t2)] +=
                            L2d(grids, tp1, tp2, Parent(A(n, l)), l - 1,
                                Grid_Point(grids, A(n, l), t1, t2, l)) *
                            exp(-i *
                                phi(Grid_Point(grids, Parent(A(n, l)), tp1, tp2,
                                               L - l + 1),
                                    Center(L - l + 1, Child(B(n, l), c)))) *
                            previous_weights[Weight_Index(Parent(A(n, l)),
                                                          Child(B(n, l), c),
                                                          l - 1, tp1, tp2)];
                      }
                  ...
                  weights[Weight_Index(A(n, l), B(n, l), l, t1, t2)] =
                      exp(i * phi(Grid_Point(grids, A(n, l), t1, t2, l),
                                  Center(l, Child(B(n, l), c)))) *
                      previous_weights[Weight_Index(
                          Parent(A(n, l)), Child(B(n, l), c), l, t1, t2)];
                }
              }
            }
          }
        ...
        // Step 5: use equivalent weights to compute full sum. B=0 since we are
        // doing the entire integral
        for (int n = 0; n < N; n++) {
          for (int m = 0; m < N; m++) {
            x = point(X(m), Y(n));
            ...
            for (int t1 = 0; t1 < q; t1++) {
              for (int t2 = 0; t2 < q; t2++) {
                SUM[Image_Index(m, n)] +=
                    L2d(grids, t1, t2, Box(x, L), L, x) *
                    exp(-i *
                        phi(Grid_Point(grids, 0, t1, t2, 0), Center(0, 0))) *
                    weights[Weight_Index(Box(x, L), 0, L, t1, t2)];
              }
            }
            SUM[Image_Index(m, n)] =
                C * exp(i * phi(x, Center(0, 0))) * SUM[Image_Index(m, n)];
            recovered_reflectivity[Image_Index(m, n)] =
                abs(SUM[Image_Index(m, n)]);
          }
        }
        // clean up memory
        delete[] weights;
    ...
    /**
     *Manages the chebyshev grids used in the buttefly algorithm.  Assumes we
     *will need the
     * */
    class GridManager {
      ...
      class ImageRecovererButterfly : ImageRecoverer {
        ...
        // shift n to the left, so that only the last 2*(L-l) digits are non
        // Zero, then shift back so that the first 2l bits are Zero.
        inline int B(int n, int l, ostream &log) {
          return n % (int)(pow(2.0, 2 * (L - l)));
        }
        ...
        // Children of B are numbers where we append two bits
        inline int Child(int B, int c, ostream &log) { return (B << 2) + c; }
        // There are N*N*r_eps weights to index.  Here, B needs to have at most
        // 2l bits
        inline int Weight_Index(int A, int B, int l, int t1, int t2,
                                ostream &log) {
          return ((A << (2 * (L - l))) + B) * r_eps + q * t1 + t2;
        ...
        /// Find the box that p is in  at level l. l=0 means only one cell.
        // The unit box is naturally partitioned. The box that a point is in is
        // completely determined by  the first 2l bits of its components. First
        // l (from left) bits are where y is, next l are x.
        inline int Box(point p, int l, ostream &log) {
          // shifts x, and y left by l bits by multiplying by 2^l.
          int column = (int)(p.x * pow(2.0, (double)l));
          int row = (int)(p.y * pow(2.0, (double)l));
        ...
        class ChebyshevGrid {
          ...
          inline point Grid_Point(double *grids, int Box, int t1, int t2, int l,
            ...
            int B1 = Box / N;
            int B2 = Box % N;
            return point(grids[Grid_Index(B1, t1, l)],
                         grids[Grid_Index(B2, t2, l)]);
          }

          // give the chebyshev grid for [0,1] with q points, sorted from left
          // to right.
          ...
          // fill in the array with the chebyshev grid adapted to box  at level
          // l. Assumes that protogrid is already allocated.  b runs from 0 to
          // 2^level-1
          inline void Grid(double *grid, double *proto_grid, int Box, int l,
                           ostream &log) {
            double scale = pow(2, (double)-l);
            for (int t = 0; t < q; t++) {
              // shift protogrid by scale, to  grid from 0 to b.  Then shift by
              // b*scale for the
